...
export default function LoginPage() {
    ...
    const renderInput = (name: string, label: string, icon: string, type: string, placeholder: string, extraElement?: React.ReactNode) => {
        ...
        return (
            <div className="form-group" style={{ marginBottom: error ? '1.8rem' : '1.2rem', transition: 'margin 0.3s ease' }}>
                <label style={{ fontSize: '0.9rem', color: '#475569', fontWeight: 600, marginBottom: '0.4rem', display: 'block' }}>{label}</label>
                <div style={{ display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
                    <div style={{ position: 'relative', flex: 1 }}>
                        <i className={icon} style={{ position: 'absolute', left: '1rem', top: '12px', color: '#94a3b8', transition: 'color 0.3s' }}></i>
                        <input
                        ...
                        />
                        <div style={{
                            position: 'absolute', top: '100%', left: '0.2rem', marginTop: '4px', color: '#ef4444', fontSize: '0.8rem',
                            display: 'flex', alignItems: 'center', gap: '4px', opacity: error ? 1 : 0, transform: error ? 'translateY(0)' : 'translateY(-5px)',
                            pointerEvents: 'none', transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)', whiteSpace: 'nowrap'
                        }}>
        ...
        <div className="landing-body login-layout" style={{ height: '100vh', overflow: 'hidden', display: 'flex' }}>
            ...
            <div className="brand-section-left">
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '3rem', zIndex: 2 }}>
                    <svg style={{ width: '48px', height: '48px', flexShrink: 0 }} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" fill="none">
                        ...
                        <defs><linearGradient id="brandGradient" x1="0" y1="0" x2="32" y2="32"><stop offset="0%" stopColor="#6366f1"/><stop offset="100%" stopColor="#8b5cf6"/></linearGradient></defs>
                    </svg>
            ...
            <div className="login-section" style={{ flex: 1, display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '2rem' }}>
                <div className="login-card login-card-modern" style={{
                    ...
                    {globalError && (
                        <div style={{ padding: '0.6rem', background: '#fee2e2', color: '#ef4444', borderRadius: '8px', marginBottom: '1rem', fontSize: '0.85rem', display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    <form onSubmit={handleSubmit}>
                                ...
                                <div>
                                    ...
                                    {renderInput('captchaCode', '图形验证', 'fas fa-shield-alt', 'text', '右侧验证码',
                                        captchaImg ? (
                                            <img src={captchaImg} alt="验证码" onClick={fetchCaptcha}
                                                 style={{
                                                     height: '42px', width: '120px', borderRadius: '8px', cursor: 'pointer',
                                                     border: '1px solid #e2e8f0', objectFit: 'contain', background: '#fff'
                                                 }}
                                                 title="点击刷新验证码" />
                                        ) : (
                                            <div style={{ height: '42px', width: '120px', background: '#f1f5f9', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                    ...
                                    {renderInput('emailCode', '邮箱验证', 'fas fa-key', 'text', '6位验证码',
                                        <button type="button" onClick={handleSendEmailCode} disabled={countdown > 0}
                                                style={{
                                                    height: '42px', width: '120px', background: countdown > 0 ? '#e2e8f0' : 'var(--landing-primary)',
                                                    color: countdown > 0 ? '#64748b' : 'white', border: 'none', borderRadius: '8px',
                                                    fontWeight: 600, cursor: countdown > 0 ? 'not-allowed' : 'pointer', transition: 'all 0.3s', fontSize: '0.85rem'
                                                }}>
                        ...
                        <button type="submit" disabled={loading} style={{
                            width: '100%', padding: '0.9rem', background: 'linear-gradient(135deg, var(--landing-primary), var(--landing-secondary))',
                    ...
                    <div style={{ textAlign: 'center', marginTop: '1.5rem', borderTop: '1px solid #e2e8f0', paddingTop: '1.2rem' }}>
                        <a href="#" onClick={(e) => { e.preventDefault(); navigate('/'); }}
                           style={{ color: '#94a3b8', textDecoration: 'none', fontWeight: 500, display: 'inline-flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.9rem' }}>
